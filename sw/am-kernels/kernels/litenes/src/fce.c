...
int fce_load_rom(char *rom) {
  ...
  for (i = 0; i < fce_rom_header->chr_block_count; i++) {
    // printf("Copying CHR pages into MMC and PPU. i = %d\n", i);
...
void fce_init() {
  cpu_init();
  printf("cpu_init done\n");
  ppu_init();
  printf("ppu_init done\n");
  ppu_set_mirroring(fce_rom_header->rom_type & 1);
  printf("ppu_set_mirroring done\n");
  cpu_reset();
  printf("cpu_reset done\n");
}
