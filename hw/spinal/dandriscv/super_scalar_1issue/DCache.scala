...
case class DCache(p : DCacheConfig) extends Component{
  ...
  val bypass_cond_0 = True // (cpu_addr >= U"32'h1000_0000") && (cpu_addr <= U"32'h3fff_ffff")
  val bypass_cond_1 = False // (cpu_addr >= U"32'h1000_1000") && (cpu_addr <= U"32'h1000_1fff")
  val bypass_cond_2 = False // (cpu_addr >= U"32'h8000_0000")
...
case class BIUTop(val config : DCacheConfig, val axiConfig : Axi4Config) extends Component {
  ...
  dcache_ports.cmd <> biu.cpu.cmd
  dcache_ports.rsp <> biu.cpu.rsp
  biu.flush := flush
  dcache_ports.stall <> biu.stall
  ...
  // ar addr unburst
  when(bypass_read){
    dcacheReader.ar.addr := biu.cpu_bypass.cmd.addr.resize(config.addressWidth)
  }.elsewhen(dcacheReader.ar.fire){
  ...
  when(bypass_write){
    dcacheWriter.aw.addr := biu.cpu_bypass.cmd.addr.resize(config.addressWidth)
  }
  // w channel
