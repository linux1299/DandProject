...
object ExeSelEnum extends SpinalEnum(binarySequential){
  val IDLE, ALU, BJU, DIV, LSU = newElement()
}

object ScoreBoardEnum extends SpinalEnum(binarySequential){
  //   0    1    2    3    4    5
  val ARF, ISS, EXE, DLY, WBC, ROB = newElement()
}
...
// ========================= Issue =======================
case class InstrPkg() extends Bundle {
...
case class IssuePkg() extends Bundle {
  val instr_pkg    = InstrPkg()
  val src1_valid   = Bool()
...
case class ExeSrc(Type: String) extends Bundle {
  ...
  val rob_adr      = UInt(ROB_ADR_W bits)
  // alu
  val uop_alu      = (Type=="ALU") generate MicroOpAlu()
  // bju
  val imm          = (Type=="BJU" || Type=="LSU") generate Bits(64 bits)
  val uop_bju      = (Type=="BJU") generate MicroOpBju()
  val branch_pc    = (Type=="BJU") generate UInt(32 bits)
  val branch_taken = (Type=="BJU") generate Bool()
  // lsu
  val uop_lsu      = (Type=="LSU") generate MicroOpLsu()
}
