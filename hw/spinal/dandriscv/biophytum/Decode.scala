...
case class DecodeComb() extends Component {
  import MyUtils._
  ...
  val micro_op  = out(MicroOp())
  val exe_sel   = out(ExeSelEnum())
  ...
  val src2_is_imm = imm_all.i_type_imm || 
                    imm_all.s_type_imm ||
                    imm_all.u_type_imm ||
                    imm_all.j_type_imm

  // common
  micro_op.uop_com.rd_wen      := rd_wen
  micro_op.uop_com.src2_is_imm := src2_is_imm
  // alu
  micro_op.uop_alu.alu_ctrl_op.assignFromBits(alu_ctrl_op)
  micro_op.uop_alu.alu_is_word := op_is_word || op_is_wordi
  // bju
  micro_op.uop_bju.bju_ctrl_op.assignFromBits(bju_ctrl_op)
  micro_op.uop_bju.bju_rd_eq_rs1   := (rd===rs1)
  micro_op.uop_bju.bju_rd_is_link  := (rd===B(0, 5 bits) || rd===B(5, 5 bits))
  micro_op.uop_bju.bju_rs1_is_link := (rs1===B(0, 5 bits) || rs1===B(5, 5 bits))
  micro_op.uop_bju.exp_ctrl_op.assignFromBits(exp_ctrl_op)
  micro_op.uop_bju.exp_csr_addr    := csr.asUInt
  micro_op.uop_bju.exp_csr_wen     := csrrw || csrrs || csrrc
  // lsu
  micro_op.uop_lsu.lsu_ctrl_op.assignFromBits(lsu_ctrl_op)
  micro_op.uop_lsu.lsu_is_load     := load
  micro_op.uop_lsu.lsu_is_store    := store
  ...
  .elsewhen(op_is_imm || op_is_alu || op_is_lui || op_is_word || op_is_wordi){
    exe_sel := ExeSelEnum.ALU
  }
  .elsewhen(op_is_jal || op_is_jalr || op_is_branch || op_is_auipc || op_is_sys){
    exe_sel := ExeSelEnum.BJU
  }
  .otherwise{
    exe_sel := ExeSelEnum.IDLE
  }
...
case class Decode() extends Component {
  ...
  val dec_dst = Vec(master(Stream(InstrPkg())), 2)

  // ==================== inst =============================
  val decode_comb = Array.fill(2)(new DecodeComb())
  
  // ==================== stream =============================
  val dst_stream = Array.fill(2)(Stream(InstrPkg()))
  ...
  for(i <- 0 until 2){
    ...
    dst_stream(i).rs2_addr     := decode_comb(i).rs2_addr
    dst_stream(i).rd_addr      := decode_comb(i).rd_addr
    dst_stream(i).imm          := decode_comb(i).imm

    // output
    dst_stream(i) >> dec_dst(i)
  }
