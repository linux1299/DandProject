...
case class Dispatch() extends Component{
  ...
  val cpl_rd_data    = Vec(in Bits(64 bits), 2)
  ...
  val entry = new Area{
    ...
    val exe      = Vec(RegInit(False), 32)
    val wbc      = Vec(RegInit(False), 32)
    val cpl      = Vec(RegInit(False), 32)
  ...
  val rs1_cpl_data  = Vec(Bits(64 bits), 2)
  val rs2_cpl_data  = Vec(Bits(64 bits), 2)
  ...
  val rs1_cpl_sel   = Vec(Bits(2 bits), 2)
  val rs2_cpl_sel   = Vec(Bits(2 bits), 2)
  val rs1_ret_sel   = Vec(Bits(2 bits), 2)
  val rs2_ret_sel   = Vec(Bits(2 bits), 2)
  val wbc_data      = wbc_rd_data.asBits
  val cpl_data      = cpl_rd_data.asBits
  ...
  src1_valid(0) := (entry.wbc(rs1_addr(0)) && entry.wbc_fire(rs1_addr(0)) && !entry.exe(rs1_addr(0))                                                                                 ) ||
                   (entry.cpl(rs1_addr(0)) && entry.cpl_fire(rs1_addr(0)) && !entry.exe(rs1_addr(0)) && !entry.wbc(rs1_addr(0))                                                      ) ||
                   (entry.ret(rs1_addr(0))                                && !entry.exe(rs1_addr(0)) && !entry.wbc(rs1_addr(0)) && !entry.cpl(rs1_addr(0))                           ) ||
                   (entry.arf(rs1_addr(0))                                && !entry.exe(rs1_addr(0)) && !entry.wbc(rs1_addr(0)) && !entry.cpl(rs1_addr(0)) && !entry.ret(rs1_addr(0)))

  src2_valid(0) := (entry.wbc(rs2_addr(0)) && entry.wbc_fire(rs2_addr(0)) && !entry.exe(rs2_addr(0))                                                                                 ) ||
                   (entry.cpl(rs2_addr(0)) && entry.cpl_fire(rs2_addr(0)) && !entry.exe(rs2_addr(0)) && !entry.wbc(rs2_addr(0))                                                      ) ||
                   (entry.ret(rs2_addr(0))                                && !entry.exe(rs2_addr(0)) && !entry.wbc(rs2_addr(0)) && !entry.cpl(rs2_addr(0))                           ) ||
                   (entry.arf(rs2_addr(0))                                && !entry.exe(rs2_addr(0)) && !entry.wbc(rs2_addr(0)) && !entry.cpl(rs2_addr(0)) && !entry.ret(rs2_addr(0))) ||
  ...
  src1_valid(1) := (rs1_addr(1) =/= rd_addr(0) || !dis_src(0).valid) &&
                    (
                     (entry.wbc(rs1_addr(1)) && entry.wbc_fire(rs1_addr(1)) && !entry.exe(rs1_addr(1))                                                                                 ) ||
                     (entry.cpl(rs1_addr(1)) && entry.cpl_fire(rs1_addr(1)) && !entry.exe(rs1_addr(1)) && !entry.wbc(rs1_addr(1))                                                      ) ||
                     (entry.ret(rs1_addr(1))                                && !entry.exe(rs1_addr(1)) && !entry.wbc(rs1_addr(1)) && !entry.cpl(rs1_addr(1))                           ) ||
                     (entry.arf(rs1_addr(1))                                && !entry.exe(rs1_addr(1)) && !entry.wbc(rs1_addr(1)) && !entry.cpl(rs1_addr(1)) && !entry.ret(rs1_addr(1)))
                    )
  ...
  src2_valid(1) := ( (rs2_addr(1) =/= rd_addr(0) || !dis_src(0).valid) &&
                     (
                       ...
                       (entry.cpl(rs2_addr(1)) && entry.cpl_fire(rs2_addr(1)) && !entry.exe(rs2_addr(1)) && !entry.wbc(rs2_addr(1))                                                      ) ||
                       (entry.ret(rs2_addr(1))                                && !entry.exe(rs2_addr(1)) && !entry.wbc(rs2_addr(1)) && !entry.cpl(rs1_addr(1))                           ) ||
                       (entry.arf(rs2_addr(1))                                && !entry.exe(rs2_addr(1)) && !entry.wbc(rs2_addr(1)) && !entry.cpl(rs2_addr(1)) && !entry.ret(rs2_addr(1))) 
                     )
                   )   ||
  ...
  rs1_cpl_sel(0)  := ((rs1_addr(0)===cpl_rd_addr(1)) && cpl_rd_wen(1)) ? B(2, 2 bits) | B(1, 2 bits)
  rs2_cpl_sel(0)  := ((rs2_addr(0)===cpl_rd_addr(1)) && cpl_rd_wen(1)) ? B(2, 2 bits) | B(1, 2 bits)
  rs1_cpl_sel(1)  := ((rs1_addr(1)===cpl_rd_addr(1)) && cpl_rd_wen(1)) ? B(2, 2 bits) | B(1, 2 bits)
  rs2_cpl_sel(1)  := ((rs2_addr(1)===cpl_rd_addr(1)) && cpl_rd_wen(1)) ? B(2, 2 bits) | B(1, 2 bits)
  ...
  rs1_cpl_data(0) := dataMux(rs1_cpl_sel(0), cpl_data)
  rs2_cpl_data(0) := dataMux(rs2_cpl_sel(0), cpl_data)
  rs1_cpl_data(1) := dataMux(rs1_cpl_sel(1), cpl_data)
  rs2_cpl_data(1) := dataMux(rs2_cpl_sel(1), cpl_data)
  ...
  for(i <- 1 until 32){
    ...
    entry.cpl_fire(i) := cpl_rd_wen(0) && cpl_rd_addr(0)===U(i) ||
                         cpl_rd_wen(1) && cpl_rd_addr(1)===U(i)
    ...
    // complete
    when(entry.wbc_fire(i)){
      entry.cpl(i) := True
    }
    .elsewhen(entry.cpl_fire(i) || entry.exe_fire(i)){
      entry.cpl(i) := False
    }

    // ret
    when(entry.cpl_fire(i)){
      entry.ret(i) := True
    }
    .elsewhen(entry.ret_fire(i)){
      entry.ret(i) := False
  ...
  for(i <- 0 until 5){
    ...
    exe_stream(i).entry_adr    := (exe_oh_valid(1)(i) && dis_older_vld && dis_newer_vld) ? tail_adr_newer | tail_adr_older // if dispatch 2 instr, use newer entry_adr
    ...
    when(exe_oh_valid(0)(i)){
      when(entry.wbc(rs1_addr(0))){
        exe_stream(i).src1_data := rs1_wbc_data(0)
      }
      .elsewhen(entry.cpl(rs1_addr(0))){
        exe_stream(i).src1_data := rs1_cpl_data(0)
      }
    ...
    .otherwise{
      ...
      .elsewhen(entry.cpl(rs1_addr(1))){
        exe_stream(i).src1_data := rs1_cpl_data(1)
      }
    ...
    when(exe_oh_valid(0)(i)){
      when(dis_src(0).iss_pkg.micro_op.uop_com.src2_is_imm && dis_src(0).iss_pkg.exe_sel=/=ExeSelEnum.LSU){
        exe_stream(i).src2_data := dis_src(0).iss_pkg.imm
      }
      .elsewhen(entry.wbc(rs2_addr(0))){
        exe_stream(i).src2_data := rs2_wbc_data(0)
      }
      .elsewhen(entry.cpl(rs2_addr(0))){
        exe_stream(i).src2_data := rs2_cpl_data(0)
      }
    ...
    .otherwise{
      when(dis_src(1).iss_pkg.micro_op.uop_com.src2_is_imm && dis_src(1).iss_pkg.exe_sel=/=ExeSelEnum.LSU){
        exe_stream(i).src2_data := dis_src(1).iss_pkg.imm
      }
      .elsewhen(entry.wbc(rs2_addr(1))){
        exe_stream(i).src2_data := rs2_wbc_data(1)
      }
      .elsewhen(entry.cpl(rs2_addr(1))){
        exe_stream(i).src2_data := rs2_cpl_data(1)
      }
  ...
  read_regfile(1).rs1_addr := rs1_addr(1)
  read_regfile(1).rs2_addr := rs2_addr(1)
